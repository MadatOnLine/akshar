...
export function registerHandlers(io: SocketServer, socket: Socket): void {
  ...
  // --- Join Group Room ---
  socket.on('join-room', async (data: { groupId: string }) => {
    const { groupId } = data;
  ...
  socket.on('send-message', async (data: {
    groupId: string;
    ciphertext: { nonce: string; tag: string; val: string };
    typingMs?: number;
    plaintext?: string; // Sent alongside for AI classification (not stored on server)
  }) => {
  ...
  socket.on('publish-key', (data: { publicKey: string }) => {
  ...
  socket.on('share-to-feed', async (data: {
    ...
    const post = await feed.shareToFeed(
      userId,
