...
The latest iteration of the TrueP2PMessenger embedded in the `akshar` protocol incorporates several major architectural upgrades to handle high-concurrency routing and AI integrations, adapting the Zero-Knowledge philosophy for a hybrid client-server topology:
...
5. **Asynchronous ML Feeding & Zero-Knowledge Compromise**: Plaintext UI messages trigger non-blocking ML vector analysis on the mesh backend. To prevent blocking the critical P2P routing path, messages are broadcast instantly with a `{ verdict: 'Pending' }` state. The AI classification resolves asynchronously and emits a `message-classified` event for optimistic UI merging.
   > **⚠️ Architectural Compromise**: Unlike the True P2P prototype (which is strictly mathematical Zero-Knowledge everywhere), the `akshar-app` implementation breaks strict Zero-Knowledge in transit by sending the `plaintext` alongside the ciphertext over the WebSocket connection to the mesh server, exclusively for AI analysis. While the plaintext is never written to CouchDB (only the blind ciphertext is stored), the mesh backend temporarily holds the plaintext in memory, meaning the server must be trusted not to log or store it.
